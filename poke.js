...
function randomBothPoke() {
	randomPoke(true, true);
}
function randomFirstPoke() {
	randomPoke(true, false);
}
function randomSecondPoke() {
...
function randomPoke(should_rand1, should_rand2) {
	...
	if (should_rand1 || num1 == undefined) {
		rand1 = getRandomPokeID();
	} else {
		rand1 = num1 - 1;
	}
	if (should_rand2 || num2 == undefined) {
...
function fuseBothPoke() {
	...
	if (!nameFix.includes(mon1) && !nameFix.includes(mon2)) {
		...
		var fmon1 =
			nameException[nameFix.indexOf(mon1)].charAt(0).toUpperCase() +
			nameException[nameFix.indexOf(mon1)].slice(1);
		var fmon2 = mon2.charAt(0).toUpperCase() + mon2.slice(1);
	} else if (!nameFix.includes(mon1) && nameFix.includes(mon2)) {
		var fmon1 = mon1.charAt(0).toUpperCase() + mon1.slice(1);
		var fmon2 =
			nameException[nameFix.indexOf(mon2)].charAt(0).toUpperCase() +
			nameException[nameFix.indexOf(mon2)].slice(1);
	} else if (nameFix.includes(mon1) && nameFix.includes(mon2)) {
		var fmon1 =
			nameException[nameFix.indexOf(mon1)].charAt(0).toUpperCase() +
			nameException[nameFix.indexOf(mon1)].slice(1);
	...
	document.getElementById('spatk1').innerHTML =
		'SPE.ATK: ' + Math.floor(spatk1) + L4[3];
	document.getElementById('spdef1').innerHTML =
		'SPE.DEF: ' + Math.floor(spdef1) + L4[4];
	...
	document.getElementById('spatk2').innerHTML =
		'SPE.ATK: ' + Math.floor(spatk2) + L5[3];
	document.getElementById('spdef2').innerHTML =
		'SPE.DEF: ' + Math.floor(spdef2) + L5[4];
	...
	var hiddenAbilities1 = fusionHiddenAbilities(
		mon1abilities,
		mon2abilities,
		abilities1
	);
...
function calcShinyHue(num1, num2, hasShinyHead, hasShinyBody) {
	...
	if (
		hasShinyHead &&
		hasShinyBody &&
		num1 in shinyColorOffsetsDict &&
		num2 in shinyColorOffsetsDict
	) {
